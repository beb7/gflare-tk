...
class GFlareRobots:
	...
	def get_matching_user_agent(self, robots_txt, user_agent):
		...
		all_uas_ordered_by_specificity = [u for u in all_uas if u.lower() in user_agent.lower()]
		all_uas_ordered_by_specificity = sorted(all_uas_ordered_by_specificity, key=len, reverse=True)
	...
	def get_ua_rules(self, crawler_user_agent, robots_txt):
		...
		if matching_ua:
			...
			match = re.match(self.get_ua_pattern(matching_ua), robots_txt, re.DOTALL|re.IGNORECASE|re.VERBOSE)
			if match:
				return match.group(2).strip()
		else:
			# Extract all rules that belong to the catch-all User-agent: *
			pattern = self.get_ua_pattern('*')
			match = re.match(pattern, robots_txt, re.DOTALL|re.IGNORECASE|re.VERBOSE)
			if match: 
				return match.group(2).strip()
		return ''
